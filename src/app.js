...
function canDragCard(){ return isViewerTurn() && game.drawnCard!==null && (game.phase==='must-swap'||(game.phase==='deck-choice'&&!revealMode)); }
function clearCardDrag(returnCard=false){
  if(!cardDrag) return;
  const {card,target}=cardDrag;
  card.style.removeProperty('--drag-x'); card.style.removeProperty('--drag-y'); card.style.removeProperty('--drag-rotate');
  card.classList.remove('dragging'); target?.classList.remove('drop-target');
...
function startCardDrag(event){
  ...
  cardDrag={card,x:event.clientX,y:event.clientY,target:null,targetIndex:null};
  card.setPointerCapture?.(event.pointerId); event.preventDefault();
}
function moveCardDrag(event){
  if(!cardDrag) return;
  const dx=Math.max(-110,Math.min(110,event.clientX-cardDrag.x));
  const dy=Math.max(-100,Math.min(180,event.clientY-cardDrag.y));
  const next=document.elementFromPoint(event.clientX,event.clientY)?.closest('#board .card[data-index]:not(.removed)') || null;
  if(next!==cardDrag.target){ cardDrag.target?.classList.remove('drop-target'); next?.classList.add('drop-target'); cardDrag.target=next; cardDrag.targetIndex=next?Number(next.dataset.index):null; if(next) haptic(8); }
  cardDrag.card.classList.add('dragging'); cardDrag.card.style.setProperty('--drag-x',`${dx}px`); cardDrag.card.style.setProperty('--drag-y',`${dy}px`); cardDrag.card.style.setProperty('--drag-rotate',`${dx/12}deg`);
  document.body.classList.toggle('dragging-card',Boolean(next));
}
function finishCardDrag(){
  if(!cardDrag) return;
  const {targetIndex}=cardDrag; clearCardDrag(targetIndex===null);
  if(targetIndex!==null){ suppressCardClick=true; handleCard(targetIndex); setTimeout(()=>suppressCardClick=false,0); }
}
...
els.drawnCard.addEventListener('pointerdown',startCardDrag);
els.drawnCard.addEventListener('pointermove',moveCardDrag);
els.drawnCard.addEventListener('pointerup',finishCardDrag);
els.drawnCard.addEventListener('pointercancel',()=>clearCardDrag(true));
