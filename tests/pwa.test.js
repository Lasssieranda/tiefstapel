...
test('Eigenständiges Premium-Kartendesign hat klare Zustände, Wertfarben und Eckzahlen', async () => {
  ...
  assert.match(app, /drawnCard\.addEventListener\('pointerdown'/);
  for (const token of ['.card.dragging','.drop-target','body.dragging-card','#discard-stage']) assert.ok(css.includes(token), `Ziehgeste-Stil fehlt: ${token}`);
  ...
  assert.match(app, /engine\.js\?v=306/);
  assert.match(sw, /tiefstapel-v12/);
  assert.match(sw, /src\/app\.js\?v=306/);
  assert.match(sw, /manifest\.webmanifest\?v=306/);
  assert.match(sw, /icons\/icon-192\.png\?v=306/);
  assert.match(sw, /icons\/icon-512\.png\?v=306/);
  assert.match(html, /src\/app\.js\?v=306/);
  assert.match(html, /manifest\.webmanifest\?v=306/);
  assert.match(html, /icons\/icon-192\.png\?v=306/);
  assert.ok(manifest.icons.every(icon => icon.src.endsWith('?v=306')));
